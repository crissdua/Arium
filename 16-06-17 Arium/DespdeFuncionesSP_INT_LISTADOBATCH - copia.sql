...
,'Vigente' AS "Estado del Documento"  
FROM "_SYS_BIC"."AddonFACE/BATCH1" 
where IFNULL("U_ESTADO_FACE",'P') in ('P','R') and "DocDate" between :FechaIni and  :FechaFin and "Series" = :Serie 
union
select 
...
"DocTotal" as "Total Documento"
,'Vigente' AS "Estado del Documento"   
FROM "_SYS_BIC"."AddonFACE/BATCH2" 
where IFNULL("U_ESTADO_FACE",'P') in ('P','R') and "DocDate" between :FechaIni and  :FechaFin and "Series" = :Serie
...
,'Vigente' AS "Estado del Documento"  
FROM "_SYS_BIC"."AddonFACE/BATCH1" 
where IFNULL("U_ESTADO_FACE",'P') in ('P','R') and "DocDate" between :FechaIni and  :FechaFin and "Series" in (select "U_SERIE" from "@FACE_RESOLUCION" where "U_ES_BATCH"='Y') 
union
select 
case IFNULL("U_ESTADO_FACE",'P') when 'P' then 'Pendiente' when 'R' then 'Rechazado' when 'A' then 'Autorizado' end AS "Estado del Documento",
"DocEntry" AS "Correlativo", 
